...
def crawling_youtube_bs4(search_word: str, count: int) -> List[List[str]]:
    start = time.time()
    cookies = {
...
def shopping_crawling_request(search_word: str, count: int) -> List[List[str]]:
    ...
    response = requests.post('https://www.coupang.com/np/search', params=params )
...
shopping_crawling_request('커피', 10)