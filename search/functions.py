...
def youtube_crawling(search_word: str, count: int) -> Union[List[List[str]], str]:
    try:
        ...
        response = requests.post("https://www.youtube.com/youtubei/v1/search", params=params, cookies=cookies, json=json_data, timeout=2)
...
def shopping_crawling(search_word: str, count: int) -> Union[List[List[str]], str]:
    try:
        ...
        response = requests.post("https://www.coupang.com/np/search", params=params, timeout=3)
