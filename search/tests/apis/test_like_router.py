...
class TestLikeCancelService(TestCase):
    ...
    def test_like_news_already_news(self) -> None:
        ...
        content = "news_content"
        thumbnail = "news_thumbnail"
        News.objects.create(title=title, user=self.user, date=date, link=link, company=company, content=content, thumbnail=thumbnail)
    ...
    def test_like_book_already_like(self) -> None:
        ...
        Book.objects.create(user=self.user, title=title, link=link, price=price, company=company, thumbnail=thumbnail, series=series, author=author)

        # When
        response = self.client.post(
            f"/api/like/book",
            data={"title": title, "link": link, "price": price, "company": company, "thumbnail": thumbnail, "series": series, "author": author},
        )
    ...
    def test_like_shopping_already_like(self) -> None:
        ...
        Shopping.objects.create(user=self.user, title=title, link=link, price=price, thumbnail=thumbnail)
