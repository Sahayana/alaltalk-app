...
class LikeCancelTest(TestCase):
    ...
    def test_like_cancel_youtube(self):
        ...
        Youtube.objects.create(user=self.user, url=test_youtube_link, title=test_youtube_title, views=test_youtube_view)

        # When
        # Content-type은 JS 에서 json이라고 보내지 않았다면 적지 않기
        response = self.client.post(
            f"/api/like_cancel/youtube",
            data={"url": test_youtube_link, "title": test_youtube_title, "views": test_youtube_view},
        )
    ...
    def test_like_cancel_news(self):
        ...
        content = "content"
        thumbnail = "thumbnail"
        News.objects.create(user=self.user, title=title, date=date, company=company, content=content, link=link, thumbnail=thumbnail)

        # When
        # Content-type은 JS 에서 json이라고 보내지 않았다면 적지 않기
        response = self.client.post(
            f"/api/like_cancel/news",
            data={
                "title": title,
                "date": date,
                "company": company,
    ...
    def test_like_cancel_book(self):
        ...
        Book.objects.create(user=self.user, title=title, price=price, company=company, author=author, link=link, thumbnail=thumbnail, series=series)

        # When
        # Content-type은 JS 에서 json이라고 보내지 않았다면 적지 않기
        response = self.client.post(
            f"/api/like_cancel/book",
            data={
                "title": title,
                "price": price,
                "company": company,
