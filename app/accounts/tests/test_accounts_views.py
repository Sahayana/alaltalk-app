...
class TestAccountsViews(TestCase):
    ...
    def test_send_friend_request_already_sent(self) -> None:
        ...
        friend_request, created = send_friend_request(
            sender_id=request.user.id, recevier_id=user2.id
        )
    ...
    def test_accept_friend_request(self) -> None:
        ...
        user2 = self.user2
        friend_request, created = send_friend_request(
            sender_id=user2.id, recevier_id=request.user.id
        )
    ...
    def test_decline_friend_request(self) -> None:
        ...
        response = self.client.get(
            reverse(
                "accounts:decline_request", kwargs={"request_id": friend_request.id}
            )
        )
        friend_requests = FriendRequest.objects.all()
