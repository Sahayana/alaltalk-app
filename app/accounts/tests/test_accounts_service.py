...
class AccountsTest(TestCase):
    ...
    def test_check_if_an_email_is_already_exists(self) -> None:
        ...
        nickname = "testuser1"
        password = "testuser1"
        create_single_user(email=email, nickname=nickname, password=password)
    ...
    def test_send_friend_request(self) -> None:
        ...
        # Given
        sender = create_single_user(
            email="test@test.com",
            nickname="testuser1",
            password="testuser1",
            bio="testuser1",
        )
        receiver = create_single_user(
            email="test2@test.com",
            nickname="testuser2",
            password="testuser2",
            bio="testuser2",
        ...
        # When
        friend_requests, is_created = send_friend_request(
    ...
    def test_send_duplicated_friend_request(self) -> None:
        ...
        friend_request, _ = send_friend_request(
            sender_id=sender.id, recevier_id=receiver.id
        )

        # When
        friend_request2, is_created = send_friend_request(
            sender_id=sender.id, recevier_id=receiver.id
        )
    ...
    def test_accept_friend_request(self) -> None:
        ...
        friend_request, is_created = send_friend_request(
            sender_id=sender.id, recevier_id=receiver.id
        )
    ...
    def test_service_accounts_token_authenticated_when_user_is_none(self) -> None:
        ...
        # When
        token = accounts_token_authenticated(
            user_email=unvalid_email, user_password=password
        )

        # Then
        self.assertEqual("NOT_REGISTERD", token)

    def test_service_accounts_token_authenticated_when_user_is_not_activated(
        self,
    ) -> None:
    ...
    def test_service_accounts_token_authenticated_when_user_is_activated(self) -> None:
        ...
        # When
        token = accounts_token_authenticated(
            user_email=user.email, user_password="testuser1"
        )
    ...
    def test_accounts_search_friends(self) -> None:
        ...
        self.assertEqual(
            user.friends.all().first().id,
            CustomUser.objects.get(nickname="testuser15").id,
        )
